...
int main(int argc, char** argv) 
{
  ...
  //do hashing one block at a time and combine the hash values in a loop
  //
  //<computed hash value + hash value from left child + hash from right child>
  uint32_t hash = 0, hashBlock = 0;
  char hashString[100];
  hash = jenkins_one_at_a_time_hash(buffer, BSIZE); //firstblock
  for(int i = 1; i < nblocks; i++){//remaining blocks
	hashBlock = jenkins_one_at_a_time_hash(&buffer[i*BSIZE], BSIZE);
	//combine hash and hashBlock using hash
